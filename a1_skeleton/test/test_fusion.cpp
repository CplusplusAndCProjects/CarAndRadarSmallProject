...
TEST (DataFusionTest, MultiSonarIntersects) {
    //  Parameters laser
    //  res: 30.0000
    //  pose: 0.0000, 0.0000, 0.0000
    //  ranges: 3.28502,3.06601,7.90626,0.49436,7.10431,7.32364,6.41023,
    ...
    RangerMockLaser r0(180, 30, {0.0,0.0,0.0}, {3.28502,3.06601,7.90626,0.49436,7.10431,7.32364,6.41023});
    RangerMockLaser r1(20, 20, {0.0,0.0,0.0}, {1.75965});
    RangerMockSonar r2(20, 20, {0.0,0.0,20}, {1.75965});
    std::vector<Cell*> cells;
    cells.push_back(new Cell());
    cells.at(0)->setCentre(0.0000,1.7597);
    cells.at(0)->setSide(0.2);

    std::vector<RangerInterface *> sensors = {&r0, &r1, &r2 };
    ...
    cout<<"scan Area by 2 sonar ="<< rf.getScanningArea();
    
    EXPECT_EQ(cell::OCCUPIED,cells.at(0)->getState());
    EXPECT_EQ(rf.getScanningArea(),rf.getScanningArea());
}
