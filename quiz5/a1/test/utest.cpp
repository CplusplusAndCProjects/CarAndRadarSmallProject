...
TEST(LaserProcessing,CountHighIntensity){
  ...
  unsigned int count = laserProcessing.countHighIntensity();
...
TEST(LaserProcessing,CountSegments){
  ...
  //! Create an object of DetectCabinet class as we will use the public function of that object to run tests against
  LaserProcessing laserProcessing(*laserScan);
  unsigned int count = laserProcessing.countSegments();

  EXPECT_EQ(count,0);

}

TEST(LaserProcessing,DetectPositon){
    //! The data has been saved in a bag, that is opened and used.
    //! Unforttunately as we need to use a ConstPtr below, we can't make this
    //! a helper function
    ...
    //! Create an object of DetectCabinet class as we will use the public function of that object to run tests against
    LaserProcessing laserProcessing(*laserScan);
    geometry_msgs::Point point = laserProcessing.detectPositionHighIntensity();

    EXPECT_NEAR(point.x,4.36, 0.1);
    EXPECT_NEAR(point.y,-3.02, 0.1);

 }

TEST(LaserProcessing,DetectPose){
    ...
    EXPECT_NEAR(pose.position.x,4.3632,0.1);
    EXPECT_NEAR(pose.position.y,-3.02028,0.1);
    EXPECT_NEAR(tf::getYaw(pose.orientation),0.215,M_PI/32);
    //You can validate these answers using the 2D Nav Goal Tool
