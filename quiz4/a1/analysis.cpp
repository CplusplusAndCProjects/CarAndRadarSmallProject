...
vector<double> Analysis::timeToImpact(Pose origin){
    ...
    //Display::V_MAX
    vector<double> times;
    Display *display = new Display(goals_.size());
    ...
    for (size_t i = 0; i < goals_.size(); i++)
    {
        ...
        double Omega_max = display->G_MAX * 9.81 / display->V_TERM; 
        double time = rangeBearingStamped.range/display->V_MAX + rangeBearingStamped.bearing/Omega_max;
