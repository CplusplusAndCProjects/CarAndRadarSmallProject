...
public:
     ...
     * @param vector of pointers to Car
     */
    Analysis(std::vector<Car*> cars);
    /**
     * @brief Constructor for class
     * @param vector of pointers to Car
     * @param raceDisplay pointer, enabling us to visualise the race
     */
    Analysis(std::vector<Car*> cars,std::shared_ptr<DisplayRace> raceDisplay);
     ...
     * @return asceding order of cars (the vector has same size as the number of cars in race), each element is their position in order using odometry (lowest position has lowest number of kilometers)
     */
