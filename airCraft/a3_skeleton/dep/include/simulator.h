...
public:
  ...
  const std::string library_version = "Version 20.9.09"; /*!< Library version*/
   ...
   *
   *  @param linear_velocityocity The linear velocity of the aircraft (metres/second)
   *  @param angular_velocityocity The angular velocity of the aircraft (radians/second)
   *
   *  @return bool - Will return false if control can not be executed based on either of 4 conditions <br>
   *                 (i) calculated gforce is more than #MAX_G <br>
   *                 (ii) the linear velocity is less than #V_TERM <br>
   *                 (iii) the linear velocity is more than #MAX_V <br>
   *                 (iv) control is supplied too frequently (more frequently than 25 ms apart)
   *
   */
  bool controlFriendly(double linear_velocityocity, double angular_velocityocity);
   ...
   *  @note This function will perform a sleep(#BSTATION_REF_RATE), to enforce
   ...
   *
   *  @return RangeBearingStamped - The range and bearing from friendly to bogies (metres) (rad), with a timestamp (milliseconds).
   *  @note This function will perform a sleep(#FRIENDLY_REF_RATE), to enforce
   *        the refresh rate of the friendly aircraft.
   *  @note The timestamp uses the simulation epoch timer as a reference.
   *  @note Data in vectoris organised such that bogies with closest range are reported first an furthest last
   */
